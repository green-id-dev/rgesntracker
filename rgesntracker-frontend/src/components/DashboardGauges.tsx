...
const DashboardGauges: React.FC<DashboardProps> = ({ approved, rejected, notApplicable, globalRate, maxValue }) => {
  return (
      ...
      <div className="flex space-x-4 overflow-auto justify-between w-full p-4">
        <div className="flex flex-col border-solid border-[1px] border-black rounded-xl px-6 bg-white items-center">
          <Gauge value={globalRate} maxValue={100} subLabel="Score de conformité" color="#00000099" bgColor='#DFDFDF99' />
        </div>
        <Gauge value={rejected} maxValue={maxValue} subLabel="Critères non conformes" color="#FF0000" bgColor='#FFE7E7' />
        <Gauge value={approved} maxValue={maxValue} subLabel="Critères conformes" color="#4AC300" bgColor='#DEFFCA' />
        <Gauge value={notApplicable} maxValue={maxValue} subLabel="Critères non applicables" color="#000000" bgColor='#DFDFDF' />
      </div>
    </GreyContainer>
  );
};
