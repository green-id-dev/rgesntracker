...
export default function Audit({criteres, audit, criteresResult, setCriteresResult, categories, setStep, setAudit}) {
    ...
    return (
        <ScrollContext.Provider value={{scrollToCategory, registerRef}}>
            ...
            <AuditMenu percents={percents} criteres={categories} date={new Date().toLocaleDateString(audit.updatedat)}
                       hours={updatedAtString} url={audit.url} saveAudit={save} onCategoryClick={handleScrollToCategory}
                       validate={validate}/>

            <DashboardGauges approved={approved.length} rejected={rejected.length} notApplicable={notApplicable.length}
                             maxValue={criteres.length} globalRate={0}></DashboardGauges>
            <OrganisationResults values={filters} onChange={handleFilterChange} criteres={criteres}
                                 criteresFilled={criteresResult} onSortChange={handleSortChange}/>
