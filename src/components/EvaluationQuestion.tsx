            ...
            <RadioGroup value={selectedOption}>
                <div className="flex gap-6 flex-wrap">
                    {options.map((option, index) => (
                        <RadioGroup.Option key={option.name} onClick={() => onChangeOption(id, index)}
                                           value={option.name} className={({checked}) =>
                  ...
                  <span className={`text-sm font-normal ${checked ? 'font-semibold' : 'text-sm'}`}>
                    {option.name}
                                    ...
                                    <span
                                        className="ml-2 flex-shrink-0 w-5 h-5 rounded-full border border-black relative">
                    {checked && (
                        <>
